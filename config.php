...
// 2) Fallback: detect from current script location
if ($sf_base_path === '') {
    $scriptName = (string)($_SERVER['SCRIPT_NAME'] ?? '');
    if ($scriptName !== '') {
        $dir = str_replace('\\', '/', dirname($scriptName));
        $sf_base_path = ($dir === '/' || $dir === '.' ) ? '' : rtrim($dir, '/');
    }
}
...
'base_path' => $sf_base_path,
    ...
    'session' => [
        ...
        'timeout' => (int) (getenv('SESSION_TIMEOUT') ?: 1800),   // 30 minutes inactivity
...
defined('PAGES_PATH') or define('PAGES_PATH', __DIR__ . '/app/pages');
