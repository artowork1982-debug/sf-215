...
`;
    ...
    function getSlotTransform(slot) {
        ...
        // Jos vanhasta järjestelmästä tullut "nollatila", käsitellään kuin ei olisi tallennettua transformia
        // jotta editorin default COVER+center pääsee ajamaan.
        if (x === 0 && y === 0 && (scale === 1 || scale === 0)) return null;
    ...
    function setSlotTransform(slot, transform) {
        const el = getEl(`sf-image${slot}-transform`);
        if (!el) return;
        el.value = JSON.stringify(transform || { x: 0, y: 0, scale: 1 });
    }
    ...
    function updateImageCardUI(slot) {
        ...
        // erillinen "Muokkaa" nappi pidetään piilossa (haluttu UI)
        const oldEditBtn = card.querySelector(`.sf-image-edit-inline-btn[data-slot="${slot}"]`);
        if (oldEditBtn) {
            oldEditBtn.classList.add('hidden');
            oldEditBtn.disabled = true;
        }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        [1, 2, 3].forEach(slot => {
            ...
            if (src && placeholder && src.indexOf(placeholder) === -1) {
                ...
                // varmista has-image oikeaan elementtiin
...
function sfClearImageSlot(slot) {
    ...
    if (transform) transform.value = JSON.stringify({ x: 0, y: 0, scale: 1 });
